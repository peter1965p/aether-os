...
import { useCartStore } from '@/modules/shop/useCartStore'; // Importiere den Store

export default function AddToCartButton({ productId }: { productId: number }) {
    const [status, setStatus] = useState<'idle' | 'loading' | 'success' | 'error'>('idle');

    // Wir holen uns die Refresh-Funktion aus deinem Store
    // HINWEIS: Prüfe in deiner useCartStore.ts, ob die Funktion dort 
    // 'fetchItems', 'refresh' oder 'load' heißt!
    ...
    const handlePress = async () => {
        ...
        try {
            ...
            if (result.success) {
                // --- DER KRITISCHE SYNC-PUNKT ---
                // Wir zwingen den Client-Store, die neuen Daten vom Server zu ziehen
                if (fetchItems) {
                    await fetchItems();
                }
            ...
            console.error("AETHER_BUTTON_FAULT:", err);
            setStatus('error');
        }
    };
