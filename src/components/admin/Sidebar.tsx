...
export default function Sidebar({ userSettings }: { userSettings?: any }) {
  ...
  const sidebarItems = [
    ...
    // --- INTELLIGENCE CORE (SEO & GEO Hier pinnen) ---
    { 
      id: 'seo', 
      label: 'SEO & GEO Intel', 
      href: '/admin/seo', 
      icon: MapPin, 
      isFree: true, // Markierung für das rote Branding
      separator: true 
    },
    ...
    <aside className={`h-screen bg-black border-r border-white/5 transition-all duration-300 flex flex-col z-50 ${isCollapsed ? "w-16" : "w-64"}`}>
      ...
      <nav className="flex-1 py-4 overflow-y-auto overflow-x-hidden custom-scrollbar">
        {sidebarItems.map((item) => {
          ...
          const isSeo = item.id === 'seo';

          return (
            <React.Fragment key={item.id}>
              {item.separator && <div className={`my-4 border-t mx-4 opacity-20 ${isSeo ? 'border-[#b33927]' : 'border-blue-500'}`} />}
              <Link
                href={item.href}
                className={`flex items-center px-4 py-3 gap-4 cursor-pointer transition-all duration-200 relative group ${
                  isActive 
                    ? (isSeo ? "bg-[#b33927]/10 text-[#b33927] border-r-2 border-[#b33927]" : "bg-blue-500/10 text-blue-400 border-r-2 border-orange-600") 
                    : "text-white/40 hover:text-white hover:bg-white/5"
                }`}
              >
                <div className={`transition-transform duration-300 ${
                  isActive 
                    ? (isSeo ? "scale-110 drop-shadow-[0_0_8px_rgba(179,57,39,0.5)]" : "scale-110 drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]") 
                    : "group-hover:scale-110"
                }`}>
                  <item.icon size={18} className={isSeo && !isActive ? "text-[#b33927]/60" : ""} />
                </div>
                ...
                {!isCollapsed && (
                  <div className="flex flex-1 items-center justify-between">
                    ...
                    {item.isFree && (
                      <span className="text-[7px] font-bold bg-[#b33927] text-white px-1.5 py-0.5 rounded-sm animate-pulse">
                        FREE
                      </span>
                    )}
                  </div>
